  ...
  return (
    <main className="min-h-screen bg-zinc-950">
        ...
        <Container>
          <div className="flex flex-wrap items-center justify-center gap-3">
            {categories.map((category, index) => (
              <button
                key={index}
                className={`px-6 py-2.5 transition-all duration-300 border-l-4 ${
                  index === 0
                    ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/30 border-l-white/20'
                    : 'bg-zinc-800/50 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 border border-zinc-700/50 border-l-transparent hover:border-l-primary-500'
                }`}
              >
                {category}
              </button>
        ...
        <Container>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post, index) => (
              <article
                key={index}
                className="group bg-gradient-to-br from-zinc-900/50 to-zinc-800/50 backdrop-blur-xl border border-zinc-700/50 overflow-hidden hover:border-primary-500/50 transition-all duration-300 cursor-pointer animate-fadeIn border-l-4 border-l-transparent hover:border-l-primary-500"
              ...
              >
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="px-3 py-1 bg-primary-500/10 border border-primary-500/30 text-xs text-primary-400 font-medium border-l-4 border-l-primary-500">
                      {post.category}
                    </span>
                  </div>
                  <h3 className="text-lg font-bold text-zinc-100 group-hover:text-primary-400 transition-colors">
                    {post.title}
                  </h3>
          ...
          <div className="text-center mt-12">
            <button className="px-8 py-4 bg-zinc-800/50 hover:bg-zinc-800 border border-zinc-700/50 hover:border-primary-500/50 text-zinc-100 font-semibold transition-all duration-300 border-l-4 border-l-zinc-600 hover:border-l-primary-500">
              Load More Posts
            </button>
          </div>
        </Container>
      ...
      <section className="py-24 bg-zinc-900/50">
        <Container>
          <div className="max-w-3xl mx-auto text-center space-y-6 bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 border border-zinc-700/50 p-12 border-l-4 border-l-primary-500/30">
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 bg-zinc-900 border border-zinc-700 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-primary-500 border-l-4 border-l-zinc-600 focus:border-l-primary-500"
              />
              <button className="px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-400 hover:to-primary-500 text-white font-semibold transition-all duration-300 border-l-4 border-white/20">
                Subscribe
              </button>
            </div>
          </div>
